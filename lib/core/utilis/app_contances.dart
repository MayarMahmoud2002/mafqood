class AppContances
{
  static const String baseUrl = 'https://project-mafqoud.000webhostapp.com';
  static const String loginPath = '$baseUrl/api/auth/login';
  static const String registerPath = '$baseUrl/api/auth/register';
  static const String restPasswordPath = '$baseUrl/api/auth/password/reset';
  static const String generateOtpCodePath = '$baseUrl/api/auth/otp/generate';
  static const String checkOtpCodePath = '$baseUrl/api/auth/otp/check';
  static const String profilePath = '$baseUrl/api/user/get';
  static const String updateProfilePath = '$baseUrl/api/user/update';
  static const String deleteProfilePath = '$baseUrl/api/user/delete';
  static const String foundedPostPath = '$baseUrl/api/foundedPerson/add';



}
